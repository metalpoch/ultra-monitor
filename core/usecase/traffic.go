...
func (use TrafficUsecase) GetTotalTrafficByState(month string) ([]*commonModel.TrafficState, error) {
	var traffics []*commonModel.TrafficState
	ctx, cancel := context.WithTimeout(context.Background(), 1*time.Minute)
	defer cancel()
		...
		if exist {
			...
			if err != nil {
				go use.telegram.SendMessage(
					constants.MODULE_TRAFFIC,
					constants.CATEGORY_DATABASE,
					fmt.Sprintf("(trafficUsecase).GetTotalTrafficByState - use.redis.Get(ctx, %s).Result()", *state),
					err,
				)
				return nil, err
			}

			var retrievedLocations model.RedisDevice
			retrievedLocations, err = utils.DeserializeModel(retrievedData)
			if err != nil {
				go use.telegram.SendMessage(
					constants.MODULE_TRAFFIC,
					constants.CATEGORY_DATABASE,
					fmt.Sprintf("(trafficUsecase).GetTotalTrafficByState - utils.DeserializeModel(%s)", retrievedData),
					err,
				)
				return nil, err
			}
			...
			if err != nil {
				go use.telegram.SendMessage(
					constants.MODULE_TRAFFIC,
					constants.CATEGORY_DATABASE,
					fmt.Sprintf("(trafficUsecase).GetTotalTrafficByState - use.repo.GetTotalTrafficByState(ctx, %v, %s)", retrievedLocations.Devices, month),
					err,
				)
				return nil, err
			}
			...
			locationsDevices := model.RedisDevice{
				Devices: devices,
			}
			...
			if err != nil {
				go use.telegram.SendMessage(
					constants.MODULE_TRAFFIC,
					constants.CATEGORY_DATABASE,
					fmt.Sprintf("(trafficUsecase).GetTrafficByODN - utils.SerializeModel(%v)", locationsDevices),
					err,
				)
				return nil, err
			}
			...
			if err != nil {
				go use.telegram.SendMessage(
					constants.MODULE_TRAFFIC,
					constants.CATEGORY_DATABASE,
					fmt.Sprintf("(trafficUsecase).GetTrafficByODN - use.redis.Set(ctx, %s, %s, %d).Err()", *state, jsonData, 24*time.Hour),
					err,
				)
				return nil, err
			}
			...
			if err != nil {
				go use.telegram.SendMessage(
					constants.MODULE_TRAFFIC,
					constants.CATEGORY_DATABASE,
					fmt.Sprintf("(trafficUsecase).GetTrafficByODN - use.redis.Get(ctx, %s).Result()", *state),
					err,
				)
				return nil, err
			}
			...
			if err != nil {
				go use.telegram.SendMessage(
					constants.MODULE_TRAFFIC,
					constants.CATEGORY_DATABASE,
					fmt.Sprintf("(trafficUsecase).GetTrafficByODN - utils.DeserializeModel(%s)", retrievedData),
					err,
				)
				return nil, err
			}
			...
			if err != nil {
				go use.telegram.SendMessage(
					constants.MODULE_TRAFFIC,
					constants.CATEGORY_DATABASE,
					fmt.Sprintf("(trafficUsecase).GetTrafficByODN - use.repo.GetTotalTrafficByState(ctx, %v, %s)", retrievedLocations.Devices, month),
					err,
				)
				return nil, err
			}
...
func (use TrafficUsecase) GetTotalTrafficByState_N(month string, n int) ([]*commonModel.TrafficState, error) {
	var traffics []*commonModel.TrafficState
	ctx, cancel := context.WithTimeout(context.Background(), 1*time.Minute)
	defer cancel()
...
func (use TrafficUsecase) GetTotalTrafficByOND(month string) ([]*commonModel.TrafficODN, error) {
	var traffics []*commonModel.TrafficODN
	ctx, cancel := context.WithTimeout(context.Background(), 1*time.Minute)
	defer cancel()
	...
	for _, odn := range odns {
		...
		if exist {
			...
			var retrievedODN model.RedisDevice
