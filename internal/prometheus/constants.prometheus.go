...
const QUERY_OLT_BY_STATES_IN string = `sum(rate(hwGponOltEthernetStatisticReceivedBytes_count{state="%s"}[15m]) * 8) by (instance)
	* on (instance) group_left(sysName) sysName`
const QUERY_OLT_BY_STATES_OUT string = `sum(rate(hwGponOltEthernetStatisticSendBytes_count{state="%s"}[15m]) * 8) by (instance)
	* on (instance) group_left(sysName) sysName`
const QUERY_OLT_BY_STATES_IFSPEED string = `sum(ifSpeed{state="%s"}) BY (instance) * on (instance) group_left(sysName) sysName`
