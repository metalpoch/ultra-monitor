...
func (repo *fatRepository) AllTao(ctx context.Context, page, limit uint16) ([]entity.Fat, error) {
	var res []entity.Fat
	offset := (page - 1) * limit
	query := `SELECT * FROM fats LIMIT $1 OFFSET $2`
...
func (repo *fatRepository) GetByOdn(ctx context.Context, state, odn string) ([]entity.Fat, error) {
	var res []entity.Fat
	query := `SELECT * FROM fats WHERE state = $1 AND odn = $2`
	err := repo.db.SelectContext(ctx, &res, query, state, odn)
	return res, err
}

func (repo *fatRepository) GetOdnStates(ctx context.Context, state string) ([]string, error) {
	var res []string
	query := `SELECT DISTINCT fats FROM tao WHERE state = $1`
	err := repo.db.SelectContext(ctx, &res, query, state)
	return res, err
}

func (repo *fatRepository) GetOdnCounty(ctx context.Context, state, county string) ([]string, error) {
	var res []string
	query := `SELECT DISTINCT fats FROM tao WHERE state = $1 AND county = $2`
...
func (repo *fatRepository) GetOdnMunicipality(ctx context.Context, state, county, municipality string) ([]string, error) {
	var res []string
	query := `SELECT DISTINCT fats FROM tao WHERE state = $1 AND county = $2 AND municipality = $3`
...
func (repo *fatRepository) GetOdnByOlt(ctx context.Context, oltIP string) ([]string, error) {
	var res []string
	query := `SELECT DISTINCT fats FROM tao WHERE olt_ip = $1`
	err := repo.db.SelectContext(ctx, &res, query, oltIP)
	return res, err
}

func (repo *fatRepository) GetOdnOltPort(ctx context.Context, oltIP string, shell, card, port uint8) ([]string, error) {
	var res []string
	query := `SELECT DISTINCT fats FROM tao WHERE olt_ip = $1 AND shell = $2 AND card = $3 AND port = $4`
...
func (repo *fatRepository) GetAllOdn(ctx context.Context) ([]string, error) {
	var res []string
	query := `SELECT DISTINCT fats FROM tao`
